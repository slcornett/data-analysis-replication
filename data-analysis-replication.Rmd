...
```{r boxcox transformation of non-normal variables}
...
```{r p-values from ANOVA}
# Note Duration
nd.p_value

SVL_sex.F$p.value
mass_sex.F$p.value), # mass
                     nd.F = as.numeric(ND_sex.F$statistic), # note duration
                     cd.F = as.numeric(CD_sex.F$statistic), # call duration
                     df.F = as.numeric(DF_sex.F$statistic), # dominant frequency
```
...
#### Boxplots of Acoustic Variables across Different Sexes\
...
require(MASS)
DFA <- df %>% filter(Nombre = "350", "352", "Ele", )
  drop_na(DF, ND, CRR, AggE_t1, SPL)
Sex1 <- df %>% dplyr::filter(Sex != "PM") # filtering for only females and nonpregnant males
Sex2 <- df %>%  dplyr::filter(Sex != "NPM") # filtering for only females and pregnant males
lda1 <- MASS::lda(Sex ~ DF + ND + CRR + AggE_t1 + SPL, data = Sex1) 
LD1 <- lda1$scaling
...
1. What problems did you encounter?  Some of the data described in the methods isn't included in the data frame. Transformations (how, for what analyses, and which variables) not inclduded in the methods
