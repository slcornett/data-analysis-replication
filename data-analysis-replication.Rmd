...
First, I make a working dataset (df) so I do not change the original data. I also prepare three sub-datasets filtered by sex, but, since I only use the one for pregnant males (p.males), I comment out the others, so you can see how I coded them, but they don't clutter the environment nor this report.
...
```
Note: the article doesn't specify what a or b mean as p-values represented in this figure. 
...
# plotting the results of the LD1 vs LD2 (visualization)
plot(lda, # model
     main = "Replicated LDA of Advertising Call Distinctiveness by Sex",
     sub = " F = Females, PM = Pregnant males, NPM = Non-pregnant males",
     xlab = "LD1 (96.3%)", #writing in the % explained by LD1
     ylab = "LD2 (3.7%)") # writing in the % explained by LD2
